...
export default function StudyPage() {
  ...
  // Kirim jawaban ke server. rating hanya dipakai kalau jawaban benar.
  async function submitAnswer(rating?: number) {
    ...
    try {
      ...
      if (data.ok) {
        ...
        if (data.is_correct && rating === undefined) {
          // Jawaban benar tapi belum menilai -> jangan hitung skor dulu,
          // tunggu user pilih rating. (skor ditambah saat rating dikirim)
        }
      } else {
  ...
  // Langkah 1: cek jawaban (belum kirim rating).
  async function handleCheck() {
    await submitAnswer(undefined)
  }

  // Langkah 2 (hanya kalau benar): kirim rating final -> update SM-2 dengan quality tepat.
    ...
    return (
      <Shell>
        <div className="rounded-lg border border-slate-200 bg-white p-10 text-center shadow-sm">
          ...
          <h2 className="text-lg font-semibold tracking-tight text-slate-900">Sesi selesai</h2>
          ...
          <div className="mt-8 flex justify-center gap-3">
            <Link href="/dashboard" className="rounded-md border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-600 shadow-sm transition hover:border-slate-300 hover:text-slate-900">
              Lihat Progres
            </Link>
            <Link href="/" className="rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-slate-800">
              Selesai
            </Link>
          </div>
        </div>
    ...
    <Shell>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-7 shadow-sm">
        ...
        <div className="mt-6 space-y-2">
          {current.options.map((opt, i) => {
            ...
            let cls = 'flex w-full items-center gap-3 rounded-md border px-4 py-3 text-left text-sm transition'
            if (feedback) {
            ...
            return (
              <button key={opt} disabled={!!feedback} onClick={() => setSelected(opt)} className={cls}>
                <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded border text-xs font-medium ${
                  isAnswer ? 'border-emerald-300 bg-emerald-100 text-emerald-700'
                  : isWrongPick ? 'border-red-300 bg-red-100 text-red-700'
      ...
      <div className="mt-6">
        {!feedback ? (
          ...
          // BENAR -> minta rating kesulitan (menentukan quality SM-2)
          <div>
            <p className="mb-3 text-center text-sm font-medium text-emerald-600">
              Benar! Seberapa mudah kamu mengingatnya?
            </p>
            <div className="grid grid-cols-3 gap-2">
              <RateButton label="Susah" hint="Ulang lebih cepat" onClick={() => handleRate(3)} disabled={submitting} />
              <RateButton label="Sedang" hint="Jadwal normal" onClick={() => handleRate(4)} disabled={submitting} />
              <RateButton label="Mudah" hint="Ulang lebih lama" onClick={() => handleRate(5)} disabled={submitting} />
            </div>
          </div>
        ) : (
          // SALAH -> langsung lanjut (sudah dihitung q2 di server)
          <div className="flex items-center justify-between gap-4">
            <p className="text-sm font-medium text-red-600">Kurang tepat</p>
            <button
              onClick={handleNext}
  ...
  return (
    <div className="min-h-screen bg-white">
      <main className="mx-auto max-w-2xl px-6 py-12">
        <Link href="/" className="mb-8 inline-flex items-center gap-1.5 text-sm text-slate-500 transition hover:text-slate-900">
